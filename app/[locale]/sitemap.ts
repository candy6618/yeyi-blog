...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const blogRoutes = allBlogs
    ...
    .flatMap((post) => {
      ...
      if (post.language !== zhLng) {
        const alternatepostsUrl = `${siteUrl}/${zhLng}/blog/${post.slug}`
        alternatepostsUrls.push({ url: alternatepostsUrl, lang: zhLng })
      }
  ...
  const authorsRoutes = allAuthors.flatMap((author) => {
    ...
    if (author.language !== zhLng) {
      const alternateauthorsUrl = `${siteUrl}/${zhLng}/about/${author.slug}`
      alternateauthorsUrls.push({ url: alternateauthorsUrl, lang: zhLng })
    }
  ...
  const routes = ['', 'blog', 'projects', 'tags'].flatMap((route) => {
    ...
    if (route !== zhLng) {
      const alternateUrl = `${siteUrl}/${zhLng}/${route}`.replace(/\/$/, '')
      alternateUrls.push({ url: alternateUrl, lang: zhLng })
    }
